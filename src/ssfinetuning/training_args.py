...
from .dataset_utils import modify_datasets, extract_keys
import os
import shutil
import numpy as np
import gc
...
def generate_kwargs(hyperparam_dic):
    """
    Generator function for all combinations of hyperparameters from
    ...
    flatten_key_vals = (
        [[kys, vals] for kys, val_list in hyperparam_dic.items()
         for vals in val_list])

    for combs in itertools.combinations(
            np.arange(len(flatten_key_vals)), numkeys):

        kys = np.array(flatten_key_vals)[list(combs)][:, 0]

        if len(set(kys)) == len(kys):
            kwargs = {flatten_key_vals[i][0]: flatten_key_vals[i][1]
                      for i in combs}
            yield kwargs
...
def with_labeled_fraction(basefunction, labeled_fraction, *args, **kwargs):
    """
    Wrapper function around train_with_ssl implemented if a list of
    labeled_fractions is mentioned.
    ...
    labeled_fraction (:obj:`list`): List of the labeled fraction of training
    data to be analysed.This function uses ~dataset_utils.modify_datasets to
    divide the dataset into the fraction of labeled dataset and unlabeled
    dataset. Then, each l_fr mentioned in labeled_fraction is analysed
    ...
    for l_fr in labeled_fraction:
        ...
        dataset_new = modify_datasets(dataset.copy(), labeled_fr=l_fr,
...
def wrapper_for_l_fr(func):
    ...
    def decorator(*args, **kwargs):
        ...
        if 'labeled_fraction' in kwargs.keys() or (
                len(args) and isinstance(args[0], list)):
...
def train_with_ssl(dataset=None,
    ...
    """
    Function for training with semisupervised models during finetuning of the
    pretrained transformer models.

    Args:

    labeled_fraction (:obj:`list` ): Set up by wrapper_for_l_fr. List of
    labeled fraction of training data to be analysed. In this case original
    dataset is divided into the fraction of labeled dataset and unlabeled
    dataset.
    ...
    run_sup (:obj:`bool`): Whether to run a supervised model along with the
    semi supervised model for comparison.

    use_sup (:obj:`bool`): Whether to use the trained supervised as the
    starting point the ssl model. If this is True, run_sup has to be true too.

    remove_dirs (:obj:`bool`): Whether to remove dirs created by
    ~transformers.Trainer.

    teacher_student_name (:obj:`Tuple[`str`, `str`]): A Tuple for teacher and
    student name, respectively for multi transformer models like MeanTeacher
    or NoisyStudent. Similar to model_name.

    num_labels (:obj:`int`): Total number of classes for the classification.

    unsup_hp (:obj:`dict`): The dictionary of all the hyperparameters in
    unsupervised part of model. Check the documentation of ssl_model_type and
    the associated trainers before setting up this dictionary. The
    train_with_ssl with then train the model on all the combinations of the
    hyperparameters set in this dictionary.

    sup_stats (:obj:`list`): Used by the wrapper as an argument. List to save
    the supervised models stats for comparison, if the run_sup is turned on.
    If labeled_fraction is not mentioned it is created within this function.

    stats (:obj:`list`): Used by the wrapper as an argument. List to save the
    chosen semisupervised models (ssl_model_type) stats. Similar to sup_stats.

    l_fr(:obj:`float`): Used by the wrapper as an argument. Float value of
    fraction used as labeled dataset.

    kwargs: Remaining dictionary arguments for the transformer.Trainer init
    function. Some of the Trainer keyword are important for training like
    compute metrics, and tokenizer, (see ~transformer.Trainer). If they are
    not mentioned, the default values would be picked, see
    default_args.set_default_args().

    Note: ~transformer.TrainingArgument which ~transformer.Trainer accepts as
    the args, here could be given in same way. To distinguish arguments for
    the supervised trainer, it should be named args_ta_sup for the supervised
    trainer and args_ta for the semisupervised trainer. There are some default
    keys set in default_args file. If one just needs to change only some args
    and keep rest of them as same default, then you can also set args_ta or
    args_ta_sup as a dictionary.

    For example setting:
    args_ta_sup = {learning_rate: 1} will only change learning rate to 1. Keep
    rest of them similar to what has been set in default_args.

    Return:

    sup_stats (:obj:`list`): If used directly without the labeled_fraction and
    if run_sup is Trueelse returns empty list. Information of all the training
    history for supervised model.

    stats (:obj:`list`): If used directly without the labeled_fraction.
    Information of all the training history for semi supervised model.
    ...
    if use_sup:
        ...
        if ssl_model_type in ['MeanTeacher', 'NoisyStudent']:
            raise TypeError(
                'Supervised models cannot be used for multi semisupervised'
    ...
    for kwargs_ssl in generate_kwargs(unsup_hp):
        ...
        if l_fr:
            logging_dir += f'runs/l_fr_{l_fr}_'

        # For tensorboard.
        for k, v in kwargs_ssl.items():
            logging_dir += k + '_' + str(v) + '_'
        ...
        trainer_ssl = MAPPING_TO_TRAINER[ssl_model_type](
            **kwargs_ssl,
            model=model_ssl,
            args=args_ta,
            dataset=dataset,
        ...
        for kys, vals in kwargs_ssl_copy.items():
            trainer_ssl.state.log_history.append((kys, vals))

        if l_fr:
            trainer_ssl.state.log_history.append(('l_fr', l_fr))
