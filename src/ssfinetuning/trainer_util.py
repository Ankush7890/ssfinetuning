...
def get_linear_schedule_with_minlr(optimizer: Optimizer, num_warmup_steps: int,
                                   ...
                                   last_epoch: int = -1, min_lr: int = 1e-07):
    """
    Creates a scheduler with a learning rate that linearly decreases but
    saturates at min_lr value.
    ...
    Return:
        :obj:`torch.optim.lr_scheduler.MultiplicativeLR` with the
...
class RemoveUnusedColumnMixing:
    ...
    def _remove_unused_columns(
        ...
        """
        Simply replacing model.pretrained_model.forward and
        model.teacher.forward in the case of MeanTeacher and NoisyStudent from
        model.forward in ~transformers.Trainer._remove_unused_columns.
        """

        if not self.args.remove_unused_columns:
            return
        # Inspect model forward signature to keep only the arguments it
        # accepts.
        if(self.model.type_ == 'MeanTeacher' or
                self.model.type_ == 'NoisyStudent'):
        ...
        # Labels may be named label or label_ids, the default data collator
        # handles that.
...
class UWScheduler:
    ...
    PiModel, TemporalEnsembling model, and Mean Teacher. In this
    implementation, it's  based on the composition with learning scheduler
    from pytorch and it works best ~transformers.Trainer without having to
    rewrite train method. # TODO: cleaner version with the rewritten
    ...
    max_w (:obj:`float`): maximum value of weight that the unsup_weight from
    model could reach.

    update_teacher_steps (:obj:`int`): useful for MeanTeacher, sets the
    interval after which teacher variables should be updated.

    w_ramprate (:obj:`float`): linear rate at which the unsupervised weight
    would be increased from the initial value.

    update_weights_steps (:obj:`int`): interval steps after which unsupervised
    weight would be updated by the w_ramprate.

    Class attributes:

    -**step_in_epochs**: Number of steps (batch passes) in an epoch.

    -**local_step**: keeps track of the times unsupervised weight has been
     changed.

    """

    def __init__(self, lr_scheduler, trainer, unsup_start_epochs=0, max_w=1,
                 update_teacher_steps=False, w_ramprate=1,
        ...
        self.steps_in_epoch = len(
            self.trainer.train_dataset) // self.trainer.args.train_batch_size

        self.update_teacher_steps = update_teacher_steps\
            if update_teacher_steps\
            else self.steps_in_epoch

    def step(self):
        """
        Implementation of composition of the pytorch learning rate scheduler
        step function with schedule of unsupervised weights. Also implements
        updating the memory logits for TemporalEnsembleModel and updating
        teacher variables for MeanTeacher model.
        ...
        if self.trainer.model.type_ == "TemporalEnsembleModel"\
            ...
            self.trainer.model.update_memory_logits(
                int(self.trainer.state.epoch + 1))
    ...
    def is_true(self, value):
        """
        A simple checker function to if it is time to update things depending
        on the value of value.
        """
        return (self.trainer.state.global_step and (
            self.trainer.state.global_step + 1) % value == 0)
...
class TrainerWithUWScheduler(RemoveUnusedColumnMixing, Trainer):
    ...
    kwargs: dictionary arguments for the ~transformers.Trainer, of dataset
    used by the trainer and could also include arguments of UWScheduler.

    Note: dataset for training can be given to the trainer in two ways.

    (i) dataset: In this case, it should the naming scheme of
    dataset_utils.modify_datasets.
    ...
    def __init__(self, kwargs_uw=None, *args, **kwargs):
        ...
        self.kwargs_uw = kwargs_uw if kwargs_uw else extract_keys(
            UWScheduler, kwargs)
    ...
    def create_optimizer_and_scheduler(self, num_training_steps):
        """
        Overriden ~transformers.Trainer.create_optimizer_and_scheduler with
        integration with the UWScheduler to its Trainer.lr_scheduler object.
        """
        super().create_optimizer_and_scheduler(num_training_steps)

        self.lr_scheduler = UWScheduler(
            self.lr_scheduler, self, **self.kwargs_uw)

    def get_train_dataloader(self):
        """
        Slightly changed ~transformers.Trainer.get_train_dataloader as models
        used in Trainer do not allow for mixing of labeled and unlabeled data.
        So changing to SequentialSampler instead of RandomSampler.
    ...
    def check_for_consistency(self):
        """
        Checks if the labeled and unlabeled are present in the same minibatch,
        raises error if they are.
        """
...
class BaseForMMTrainer(RemoveUnusedColumnMixing, Trainer):
    ...
    def pre_train_init(self, num_training_steps):
        ...
        if self.use_min_lr_scheduler is not None:
            self.lr_scheduler =\
                get_linear_schedule_with_minlr(
                    self.optimizer,
    ...
    def post_epoch(self, step, epoch, tr_loss):
        ...
        after the epoch is done. See ~transformers.Trainer.train() for more
        details.
        ...
        self.state.epoch = self.global_epoch + \
            epoch + (step + 1) / self.steps_in_epoch
        self.control = self.callback_handler.on_step_end(
            self.args, self.state, self.control)
        self._maybe_log_save_evaluate(
            tr_loss, self.model, trial=None, epoch=epoch)
    ...
    def confi_prediction(self, logits_m1, logits_m2, logits_m3=None):
        ...
        if logits_m3 is None:
            ...
            m1_confi = torch.logical_and(
                p_label1 >= p_label2, p_label1 >= p_label3)
            m2_confi = torch.logical_and(
                p_label2 >= p_label1, p_label2 >= p_label3)
            m3_confi = torch.logical_and(
                p_label3 >= p_label1, p_label3 >= p_label2)
    ...
    def prediction_step(
        ...
        confi_prediction method to find prediction of Cotrain and TriTrain
        Models.
        ...
        with torch.no_grad():
            ...
            if has_labels:
                if self.label_smoother is not None and "labels" in inputs:
                    ...
                    loss = (
                        outputs["loss"] if isinstance(
                            outputs, dict) else outputs[0]).mean().detach()
                if isinstance(outputs, dict):
                    logits = {
                        k: v for k,
                        v in outputs.items() if k not in ignore_keys +
                        ["loss"]}
...
class TrainerForCoTraining(BaseForMMTrainer):
    ...
    epoch_per_cotrain (:obj:`int`, `optional`, defaults to 2): Number of
    epochs to pass through training data while going through one iteration
    of cotraining.

    exchange_threshold (:obj:`int`, `optional`, defaults to 20): Threshold
    value of exchange between the model, below which co training is stopped.

    ntimes_before_saturation (:obj:`int`, `optional`, defaults to 2): In the
    case of get_linear_schedule_with_minlr, number of times should go over
    all model dataset + unlabeled dataset before saturating.

    p_threshold (:obj:`float`): threshold probability for considering exchange
    between models.

    use_min_lr_schedule (:obj:`bool`): Whether to use
    linear_schedule_with_minlr.

    min_lr (:obj:`int`, `optional`, defaults to 1e-07): The value of minimum
    learning rate used by linear_schedule_with_minlr.

    show_exchange (:obj:`bool`): Whether to print the exchange happening
    between models.

    max_passes (:obj:`int`): Maximum number of passes through the all
    the datasets.
    ...
    (ii)dataset_model1, dataset_model2, unlabeled: dataset for model1, model2
    and unlabeled dataset respectively.


    Class attributes:

    -**total_dataset_len**: total length of all datasets including model1,
    model2 and unlabeled.
    ...
    def __init__(self, epoch_per_cotrain=2, exchange_threshold=20,
                 ntimes_before_saturation=2, p_threshold=0.65,
                 use_min_lr_scheduler=True, min_lr=1e-07,
                 show_exchange=True, max_passes=1, *args, **kwargs):

        dataset = kwargs.pop('dataset', None)

        self.dataset_model1 = dataset['labeled1'] if dataset else kwargs.pop(
            "dataset_model1")
        self.dataset_model2 = dataset['labeled2'] if dataset else kwargs.pop(
            "dataset_model2")
        self.dataset_unlabeled = dataset['unlabeled'] if dataset else\
            kwargs.pop("unlabeled")

        self.use_min_lr_scheduler = use_min_lr_scheduler
        # needed ~transformers.Trainer's constructor to not to initate the
        # lr_scheduler.
        ...
        self._remove_unused_columns(
            self.dataset_unlabeled,
            description="unlabeled")
        self._remove_unused_columns(
            self.dataset_model1,
            description="labeled1")
        self._remove_unused_columns(
            self.dataset_model2,
            description="labeled2")
    ...
    def train(self):
        ...
        num_training_steps = int(self.ntimes_before_saturation *
                                 self.total_dataset_len *
                                 self.epoch_per_cotrain /
    ...
    def cotrain(self, model1_train, model2_train):
        ...
        training dataset, see SimpleDataset.extend_length(). Then, a batch
        from both model1 and model2 dataset is passed to cotrain_forward,
        where applies the orthogonality based between the classifier layers
        and calculates the total loss from both the batches.

        Args:
        model1_train (:obj:`SimpleDataset`): Training dataset for model 1
        after the exchange_unlabeled_data has been used.
        ...
        for epoch in range(self.epoch_per_cotrain):
            ...
            for step, (batch_for_model1, batch_for_model2) in enumerate(
                    zip(model1_dataloader, model2_dataloader)):
                ...
                outputs = self.model.cotrain_forward(
                    batch_for_model1, batch_for_model2)
    ...
    def exchange_unlabeled_data(self):
        ...
        to train on and vice versa.

        Args:
        ul_dataloader (class:`~torch.utils.data.DataLoader`): dataloader for
        unlabeled dataset.

        Returns:
        exchange (:obj:`bool`): If the number of the examples exchanged are
        above the exchange_threshold.

        dataset_model1, dataset_model2 (:obj:`SimpleDataset`): dataset for
        model1 and model2 with added unlabeled data.
        ...
        for batch_index, inputs in enumerate(ul_dataloader):
            ...
            labels_m1_confi = torch.logical_and(
                p_label1 >= self.p_threshold, p_label2 < self.p_threshold)
            labels_m1_confi = torch.logical_and(
                labels_m1_confi, labels1 != labels2)

            # labels on which model 2 is confident on.
            labels_m2_confi = torch.logical_and(
                p_label2 >= self.p_threshold, p_label1 < self.p_threshold)
            labels_m2_confi = torch.logical_and(
                labels_m2_confi, labels1 != labels2)

            inputs['label'] = labels2
            added += self.dataset_model1.append(inputs,
                                                labels_m2_confi, batch_index)

            inputs['label'] = labels1
            added += self.dataset_model2.append(inputs,
                                                labels_m1_confi, batch_index)

        exchange = True if added > self.exchange_threshold else False

        if self.show_exchange:
            print(f'Number of unlabeled examples exchanged: {added.item()}')
...
class TrainerForTriTraining(TrainerForCoTraining):
    ...
    """
    Subclass of ~TrainerForCoTraining with adding of a third model for
    Tritrain Model.

    Args:

    procedure (:obj:`str`, `optional`, defaults to 'agreement'): Whether
    to train with TriTraining with agreement
    <https://ieeexplore.ieee.org/document/1512038> or with disagreement
    <https://www.aclweb.org/anthology/P10-2038/>.

    Note: dataset for training can be given to Trainer as same way as
    TrainerForCoTraining but with and addition of the third dataset for
    model3.
    ...
    def __init__(self, procedure='agreement',
                 epoch_per_tritrain=2, *args, **kwargs):

        dataset_bool = True if 'dataset' in kwargs.keys() else False

        self.dataset_model3 = kwargs['dataset']['labeled3'] if dataset_bool\
            else kwargs.pop("dataset_model3")
        ...
        self._remove_unused_columns(
            self.dataset_model3,
            description="labeled3")
        self.dataset_model3 = SimpleDataset(self.dataset_model3)

    def train(self):
        """
        Train method for TriTrainer. Performs exchange of unlabeled dataset
        between three models, model1, model2 and model3 until exchange
        condition is true, see exchange_unlabeled_data().
        """
        ...
        exchange = True
        count_passes = 0
        while exchange is True or count_passes < self.max_passes:
    ...
    def tri_train(self, model1_train, model2_train, model3_train):
        """
        Method for tri training. Same procedure as
        TrainerForCoTraining.co_train but with the addition of training for
        the third model.
        """
        ...
        for epoch in range(self.epoch_per_tritrain):
            ...
            for step, batches in enumerate(
                ...
                ct_outputs = self.model.cotrain_forward(
                    batch_for_model1, batch_for_model2)
    ...
    def disagreement_proc(self, labels_confi, la, lb, l_compare):
        """
        Function to implement disagreement procedure during the exchange of
        unlabeled data.
    ...
    def agreement_proc(self, labels_confi, la, lb, l_compare):
        """
        Function to implement agreement procedure during the exchange of
        unlabeled data.
    ...
    def exchange_unlabeled_data(self, ul_dataloader):
        ...
        chosen exchange procedure. Rest is similar to
        TrainerForCoTraining.exchange_unlabeled_data().
        ...
        for batch_index, inputs in enumerate(ul_dataloader):
            ...
            labels_m1m2_confi = torch.logical_and(
                p_label1 >= self.p_threshold, p_label2 >= self.p_threshold)
            labels_m1m2_confi = self.exchange_proc(
                labels_m1m2_confi, labels1, labels2, labels3)

            labels_m2m3_confi = torch.logical_and(
                p_label2 >= self.p_threshold, p_label3 >= self.p_threshold)
            labels_m2m3_confi = self.exchange_proc(
                labels_m2m3_confi, labels2, labels3, labels1)

            labels_m1m3_confi = torch.logical_and(
                p_label1 >= self.p_threshold, p_label3 >= self.p_threshold)
            labels_m1m3_confi = self.exchange_proc(
                labels_m1m3_confi, labels1, labels3, labels2)
            ...
            inputs['label'] = labels2
            added += self.dataset_model3.append(inputs,
                                                labels_m1m2_confi,
                                                batch_index)
...
class TrainerForNoisyStudent(BaseForMMTrainer):
    ...
    epoch_per_ts_iter (:obj:`int`, `optional`, defaults to 1): Number of
    epochs to pass during each teacher student iteration.
    ...
    ntimes_before_saturation (:obj:`int`, `optional`, defaults to 2): In the
    case of get_linear_schedule_with_minlr, number of times should go over all
    model dataset + unlabeled dataset before saturating.
    ...
    (ii)dataset_labeled, dataset_unlabeled: dataset for labeled and
    unlabeled data.

    """

    def __init__(self, min_lr=1e-07, epoch_per_ts_iter=1, ts_iter=3,
                 ntimes_before_saturation=2, reduce_init_lr_factor=1,
                 use_min_lr_scheduler=None, *args, **kwargs):

        dataset = kwargs.pop('dataset', None)

        self.use_min_lr_scheduler = use_min_lr_scheduler
        self.dataset_labeled = dataset['labeled'] if dataset else kwargs.pop(
            "dataset_labeled")
        self.dataset_unlabeled = dataset['unlabeled'] if dataset\
            else kwargs.pop("dataset_unlabeled")
        ...
        self.global_epoch = 0
        # repoint to transformers.Trainer.prediction_step() method for this
        # case.
    ...
    def train(self):
        ...
        every iteration. In this case both optimizer and learning rate
        scheduler at reinitiated at the end of an iteration.
    ...
    def num_training_steps_(self):
        return int(len(self.dataset_labeled) *
                   self.epoch_per_ts_iter / self.args.train_batch_size)
