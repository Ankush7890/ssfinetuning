...
def dic_to_pandas(history, loss_key='eval_loss', accuracy_measure='eval_matthews_correlation'):
    ...
    for state in history:
        ...
        for dics in state:
            ...
            if isinstance(dics, dict) and loss_key in dics.keys():
                ...
                if accuracy_measure:
                    stats[accuracy_measure].append(dics[accuracy_measure])

                enteries_in_old_dics += 1

            elif isinstance(dics, list) or isinstance(dics, tuple):
