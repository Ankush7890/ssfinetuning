...
class BaseMultiPretrained(nn.Module):
    ...
    def __init__(self,
        ...
        if teacher_dropout is not None:
            self.teacher.dropout.p = teacher_dropout

        self.student = AutoModelForSequenceClassification.from_pretrained(
            self.student_model_name, num_labels=num_labels)

        if student_dropout is not None:
            self.student.dropout.p = student_dropout
